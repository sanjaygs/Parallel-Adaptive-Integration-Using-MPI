...
double area(double  local_a, double  local_b, int local_n, double  h, int process_rank) {
    double local_area,x,y,h2=h/2;
    int i;
    y = (f(local_b/2.0) + f(local_a/2.0))/2;
    local_area = (f(local_a) + f(local_b) + 4*y)/6.0;
...
int main(int argc, char** argv) {
    ...
    double a,b,h,local_a,local_b,local_area=0.0,total,start,finish;
    MPI_Status  status;
    MPI_Init(&argc, &argv);
    ...
    local_area = area(local_a, local_b, local_n, h, process_rank);
    if (process_rank == 0) {
        total = local_area;
